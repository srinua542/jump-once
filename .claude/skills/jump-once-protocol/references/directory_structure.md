...
├── content/                        # Authored/generated campaign data (P10): schema + assembler logic + persisted payloads
│   ├── schema/                     # ChapterFramework (REQ-083 seven-step) + CampaignManifest — strict-parsed
│   └── data/                       # Persisted payloads: per-chapter framework JSON, per-level LevelDefinition JSON, campaign manifest, validation-report.json
```
...
- **Content isolation.** `content/` holds P10's authored/generated campaign data (added at P10 start, dm-0108): the `ChapterFramework`/`CampaignManifest` schema and the assembler/estimator/proof logic that produces and validates persisted payloads under `content/data/`. It consumes `src/gen/` and `src/eval/` strictly through their public entry points (`manufactureLevel`, `Concept.validateConcept`, `Creativity.evolveLevel`; `evaluateLevel`/`assembleLevelEvidence`, `AgentHarness.runAgent`/`CANONICAL_PIPELINE`, `Optimization.computeOptimizationWindow`, `campaign/`'s `processCampaign`/`updateState`, `macro/Curriculum.validateCurriculum`) — never gate/search internals, never constructs a `LevelDefinition` directly outside that pipeline. The dependency is one-way — nothing under `src/`, `tools/`, or `render/` may ever import from `content/`. Enforced by `test/unit/ContentIsolation.test.ts`.
