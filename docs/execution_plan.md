...
- **P2 — Data Models & Level Definition Schema: ACTIVE.** Plan authored at the start of S2.1 (this session); see the P2 section below.
...
# P2 — Data Models & Level Definition Schema  *(ACTIVE — authored at S2.1 start, per REQ-P02)*
...
3. **Entities are records, not classes and not an ECS.** `EntityDef = { id, kind, transform, collider, behavior }` where `behavior` is a closed discriminated union matching the §16 library (moving platform, collapsing floor, ice, spike, laser, moving hazard, plate, proximity zone, door, spring, gravity zone, conveyor). Systems (P3) key on component presence, not entity kind. Sparse-set/archetype ECS machinery is rejected as over-engineering at this entity count; entity classes are rejected as a data/logic-decoupling violation.
4. **Serialization is canonical JSON (dm-0010).** Finite IEEE doubles round-trip exactly; therefore the validator rejects non-finite numbers and normalizes `-0` to `0`, and `serializeLevel()` emits a fixed field order. Round-trip safety is defined precisely: value-level `deepEqual(parse(serialize(v)), v)` plus byte-level idempotence `serialize(parse(serialize(v))) === serialize(v)`. Byte-identity with the hand-authored source file is deliberately not required.
...
6. **Parse, don't validate.** `parseLevel(raw: unknown): Result<LevelDefinition, SchemaError[]>` is the only construction path from untrusted input; errors carry JSON-pointer-style paths. The validator is hand-rolled and dependency-free; the drift risk is priced by requiring one failing-fixture test per rejection rule.
...
- **S2.2** `src/components/LevelDefinition.ts` (or sibling) — schema root types + `serializeLevel()` canonical serializer; `docs/level_schema.md` documenting every field, the coordinate convention, the versioning policy, and the axiom boundary.
- **S2.3** Loader + structural validator — `parseLevel(raw: unknown)`, path-qualified errors, one failing-fixture test per rejection rule.
- **S2.4** Hand-authored sample level fixture in `test/fixtures/` (schema-validation scaffolding, explicitly not P10 content) + round-trip tests.
...
- **Rejection coverage:** every validator rule demonstrably rejects a bad fixture with a path-qualified error — wrong version, non-finite number, tilemap length ≠ width×height, unknown tile id, unknown entity/trigger kind, duplicate id, dangling reference, out-of-bounds spawn/goal, malformed GDOS curve. *(REQ-122)*
- **Round-trip:** value-level lossless and byte-level idempotent, per the definitions above. *(REQ-122)*
...
- **C2.2** Schema types + canonical serializer land; `docs/level_schema.md` filed; coordinate convention ledgered.
- **C2.3** Validator rejects every bad fixture and accepts the good one; all rejection paths tested.
- **C2.4** Sample level fixture round-trips (deep-equal + byte-idempotent).
- **C2.5** Deterministic instantiation + engine-consumption integration test green.
- **C2.6** `docs/verification/P2.md` filed; REQ-122/014/120/121 re-audited per dm-0008 (REQ-154 stays open — its full scope closes in P3); PKG updated with the new nodes and `pkg_hash` bumped.
